...
\href{https://github.com/IOGS-LEnsE-ressources/mbed6-libraries}{\large https://github.com/IOGS-LEnsE-ressources/mbed6-libraries} / Sections \textit{progs} et \textit{platforms}
...
Cette première séance est consacrée à la découverte de la \textbf{programmation de systèmes embarqués} (ici des cartes \textbf{Nucléo} de \textit{STMicroelectronics}, basées sur des microcontroleurs \textit{STM32}) et la prise en main de l'interface de développement \textbf{Keil Studio Cloud} (et les bibliothèques MBED).
	...
	\begin{description}
		...
		\item[Etape 5 - 45 min] Acquérir des données analogiques - Potentiomètre
		\item[Etape 6 - 30 min] Mettre en \oe{}uvre des interruptions sur des événements internes
	\end{description}	
...
Une \textbf{série d'exemples de code} (ainsi qu'un projet de base) est disponible à l'adresse suivante : 

\href{https://github.com/IOGS-LEnsE-ressources/mbed6-libraries}{https://github.com/IOGS-LEnsE-ressources/mbed6-libraries}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%    Premier programme
\section{Etape 6 / Mettre en \oe{}uvre des interruptions sur des événements internes}

Sur les microcontroleurs de type STM32, il existe un \textbf{mécanisme matériel de temporisation} qui permet d'exécuter du code de \textbf{manière périodique ou différée}, sans bloquer le programme principal.

Le \textbf{\textsc{Ticker}} est un \textbf{timer matériel} utilisé par Mbed pour mesurer le temps et déclencher des interruptions à intervalles réguliers. 
...
\begin{lstlisting}
tik.attach(&tik_ISR, 0.5);
\end{lstlisting}

où \textsl{tik\_ISR()} est une fonction, appelée \textbf{routine d'interruption}, qui sera exécutée ici toutes les 0.5 seconde.
...
Dans cet exemple, toutes les 0.5 secondes, la fonction \textsl{tik\_ISR()} est appelée. Elle lit alors l'état de la sortie \textsl{led1} pour ensuite l'inverser.

\subsection{Travail à réaliser}

On se propose de tester le code \textsl{interrupt\_on\_ticker.cpp}. Ce fichier est disponible sur le site du LEnsE dans la rubrique \textit{Année / Première Année / Interfaçage Numérique S6 / Bloc 1 Systèmes embarqués / Exemples MBED pour STM32}.

\Manip Tester et expliquer le code fourni. \textit{En traçant des chronogrammes...}

\Manip Réaliser un programme qui permet d'activer (ou de désactiver) l'action effectuée par le code fourni précédemment par l'appui sur un bouton poussoir. \textbf{Votre programme devra utiliser uniquement le principe d'interruption.}

\Manip Tester et expliquer le code suivant : \textsl{interrupt\_on\_ticker\_bonus.cpp}
...
%%| -------------- | -------------- | --------------------- |
%%| `int`          | 4 octets       | `int a = 5;`          |
%%| `float`        | 4 octets       | `float x = 3.14f;`    |
%%| `double`       | 8 octets       | `double y = 2.71828;` |
%%| `char`         | 1 octet        | `char c = 'A';`       |
%%| `bool` *(C++)* | 1 octet        | `bool ok = true;`     |
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\subsection*{Opérateurs principaux}
%
%%* **Arithmétiques** : `+ - * / %`
%%* **Logiques** : `&& || !`
%%* **Comparaisons** : `== != < > <= >=`
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\subsection*{Tableaux et chaînes}
%
%%int t[5] = {1, 2, 3, 4, 5};
%%printf("%d\n", t[0]);
%%
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\subsection*{Structures (C) et classes (C++)}
%
%%struct Point {
%%    int x;
%%    int y;
