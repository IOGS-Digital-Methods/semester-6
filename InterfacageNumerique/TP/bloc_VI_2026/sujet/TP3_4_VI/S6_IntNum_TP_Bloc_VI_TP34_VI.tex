...
VOIR METHODE POUR MASQUER EN RVB vs HSV
...
\Manip Modifier le code pour ajouter une option (par exemple la lettre 'e') : permettre l'affichage de l'image subissant une érosion avec un noyau circulaire de 7x7.

\Manip Tester votre code.

%%%%%%%%%%%%%    Etape B
\subsection{VI-B - Calibrer la chaîne pour les cubes colorés {[40 min]}}
...
FOURNIR UNE IMAGE AVEC LES CUBES ? EN ECLAIRAGE RGB - dans plusieurs configurations

\Quest Proposer une méthode (algorithme) à mettre en place pour détecter la présence d'un élément d'une des 4 couleurs à détecter.

\Manip A partir de l'exemple précédent (en remplaçant si besoin l'image provenant de la caméra par l'ouverture d'une image fixe), ajouter la possibilité de détecter les différents cubes colorés (rouge, bleu, vert, jaune).

Il pourra être intéressant d'afficher (dans la console par exemple) : les coordonnées des différents éléments, la taille de la surface concernée, la couleur 

%%%%%%%%%%%%%    Etape C
\subsection{VI-C - Calibrer la chaîne pour des formes particulières {[40 min]}}

FOURNIR UNE IMAGE EN GRAY AVEC LES DIFFERENTES FORMES ? NON-ALIGNEES PUIS ALIGNEES ?
