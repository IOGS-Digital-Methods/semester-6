...
\begin{itemize}
	\item \texttt{Color} : classe pour représenter une couleur et effectuer des conversions. Cela peut être utile manipuler les couleurs des patches.
	\item \texttt{PerspectiveRemover} : classe pour retirer la perspective d'une image à partir des quatre coins de la mire. Le but est d'obtenir une image où les patches de la mire sont alignés avec les pixels, ce qui facilitera l'extraction des couleurs.
...
\begin{itemize}
	...
	\item \textbf{Séance 2} : Découverte des classes de du fichier de démarrage. Réflexion sur la structure du code via un schéma de classes. Validation en fin de séance.
...
Pour améliorer la précision, on peut réaliser une moyenne des couleurs sur une zone autour du pixel central. La taille de cette zone est à définir, mais elle doit être suffisamment grande pour contenir une grande partie du patch, mais pas trop grande pour ne pas inclure les couleurs des patches voisins.
...
\begin{description}
	...
	\item[Ouverture C] \textbf{Graphes de modification de la chromaticité $xy$}
	      ...
	      \begin{itemize}
		      \item Il est possible de réaliser un graphe dans le diagramme de chromaticité CIE 1931 xy; pour chaque patch on peut tracer une flèche qui illustre la modification de chromaticité entre la référence et l'image, avant et après correction. Une illustration est donnée en figure \ref{chromaticity}.
	      \end{itemize}
