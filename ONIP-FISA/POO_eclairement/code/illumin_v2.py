...
class WorkingPlan:
    ...
    def process_plan_to_source_vector(self, source_index:int):
        """
        Calculate the plan to source vector - from a point of the plan.
        :param source_index: index of the light source.
        :return: 2D array (size of the plan) with the value of the PS vector.
        """
        if source_index < len(self.sources_list):
            # PS vector = xP-xS, yP-yS, zP-zS
            source_x, source_y, source_z = self.sources_list[source_index].get_coords()
    ...
    def process_alpha_source(self, source_index:int, ps_vect: np.ndarray=[]):
        """
        ...
        if source_index < len(self.sources_list):
            if len(ps_vect) == 0:
                ps_vect = self.process_plan_to_source_vector(source_index)
            # direction vector of the source
            ns = np.stack((self.sources_list[source_index].get_direction_vector()), axis=-1)
            # Alpha angle
    ...
    def get_intensity_led(self, source_index:int, alpha: np.ndarray=[]):
        """
        ...
        if source_index < len(self.sources_list):
            if len(alpha) == 0:
                alpha = self.process_alpha_source(source_index)
            return self.sources_list[source_index].get_intensity_rad(alpha)
        else:
            return None

    def process_cospsi_source(self, source_index:int, ps_vect: np.ndarray=[]):
        """
    ...
    def get_illumination_source(self, source_index:int, ps_mag:np.ndarray,
        ...
        if source_index < len(self.sources_list):
            if len(intensity) == 0:
                intensity = self.get_intensity_led(source_index)
            if len(cospsi) == 0:
                psi, ps_mag = self.process_psi_source(source_index)
    ...
    def process_global_illumination(self):
        ...
        for k in range(nb_source):
            print(f'Source {k}')
            ps_vect = self.process_plan_to_source_vector(k)
            alpha = self.process_alpha_source(k, ps_vect)
            intensity = self.get_intensity_led(k, alpha)
            cos_psi, ps_mag = self.process_cospsi_source(k, ps_vect)
            illumine = self.get_illumination_source(k, ps_mag, intensity, cos_psi)
            if k == 0:
...
if __name__ == '__main__':
    led1 = Source(100, 30, 1, 1.5, 1, 50, 30)
    led2 = Source(100, 10, 1.5, 2.5, 1, 90, 50)

    w_plan = WorkingPlan(2, 3, 0.01)
